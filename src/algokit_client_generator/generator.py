...
from algokit_client_generator.spec import ABIContractMethod, ABIStruct, ContractMethod
...
def helpers(context: GeneratorContext) -> DocumentParts:
...
def named_struct(context: GeneratorContext, struct: ABIStruct) -> DocumentParts:
    yield "@dataclasses.dataclass(kw_only=True)"
...
def typed_arguments(context: GeneratorContext) -> DocumentParts:
    # typed args classes
...
def state_type(context: GeneratorContext, class_name: str, schema: dict[str, dict]) -> DocumentParts:
    if not schema:
        return
...
def state_types(context: GeneratorContext) -> DocumentParts:
...
def composer(context: GeneratorContext) -> DocumentParts:
...
def typed_client(context: GeneratorContext) -> DocumentParts:
    yield f"class {context.client_name}:"
    yield Part.IncIndent
    yield utils.docstring(
        (f"{context.app_spec.desc}\n\n" if context.app_spec.desc else "")
        + f"""A class for interacting with the {context.app_spec.name} app providing high productivity and
...
def forwarded_client_properties(context: GeneratorContext) -> DocumentParts:
    yield utils.indented(
        """
@property
...
def signature(
    context: GeneratorContext, name: str, method: ContractMethod, *, compose_signature: bool = False
) -> DocumentParts:
    yield f"def {name}("
    yield Part.IncIndent
    yield "self,"
...
def clear_method(context: GeneratorContext) -> DocumentParts:
    yield utils.indented(
        """
def clear_state(
    self,
...
def compose_clear_method(context: GeneratorContext) -> DocumentParts:
    yield utils.indented(
        """
def clear_state(
    self,
...
def deploy_method_args(context: GeneratorContext, arg_name: str, methods: list[ContractMethod]) -> DocumentParts:
    yield Part.InlineMode
...
def deploy_method(context: GeneratorContext) -> DocumentParts:
    yield utils.indented(
        """
def deploy(
    self,
...
def get_global_state_method(context: GeneratorContext) -> DocumentParts:
...
def get_local_state_method(context: GeneratorContext) -> DocumentParts:
