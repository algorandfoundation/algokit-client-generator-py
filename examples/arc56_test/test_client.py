...
def test_arc56_demo(
    ...
    algorand.account.ensure_funded_from_environment(
        bob, AlgoAmount.from_micro_algo(10_000_000)
    )
    ...
    with pytest.raises(Exception, match="subtract.a must be greater than subtract.b"):
        client.send.foo(
            args=FooArgs(
                inputs=Inputs(
                    add=InputsAdd(a=1, b=100), subtract=InputsSubtract(a=1, b=2)
                )
            )
        )
    ...
    client.app_client.fund_app_account(
        FundAppAccountParams(amount=AlgoAmount.from_micro_algo(1_000_000))
    )
