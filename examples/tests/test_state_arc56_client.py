...
@pytest.fixture
def state_factory_arc56(algorand: AlgorandClient, default_deployer: algokit_utils.AddressWithSigners) -> StateFactory:
    return algorand.client.get_typed_app_factory(StateFactory, default_sender=default_deployer.addr)


@pytest.fixture
def deployed_state_app_client(state_factory_arc56: StateFactory) -> StateClient:
...
def test_exposes_state_correctly(
    state_factory_arc56: StateFactory, default_deployer: algokit_utils.AddressWithSigners
) -> None:
    client, _ = state_factory_arc56.deploy(
    ...
    client.state.local_state(default_deployer.addr).local_bytes_not_in_snake_case  # Should not throw KeyError
...
def test_arguments_with_defaults(state_factory_arc56: StateFactory) -> None:
    client, _ = state_factory_arc56.deploy(
        compilation_params={"deploy_time_params": {"VALUE": 1}},
    )
